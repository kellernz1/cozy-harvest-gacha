...
import { PACKS, SEED_EMOJIS, RARITY_INCOME, type Seed, type Rarity } from '../game/constants';
...
export default function Shop() {
  ...
  const handleBuyPack = (packId: number) => {
    ...
    if (state.money < pack.cost) {
      toast.error(`Not enough coins! Need $${pack.cost}`);
      return;
    }
    ...
    seeds.forEach((_, i) => {
      setTimeout(() => {
        ...
        if (i === seeds.length - 1) {
          // All revealed, add to inventory after delay
          setTimeout(() => {
            dispatch({ type: 'ADD_SEEDS', seeds });
            setRevealing(null);
            setRevealedCount(0);
            toast.success(`Got ${seeds.length} seeds!`);
          }, 1200);
        }
      }, (i + 1) * 600);
    });
  ...
  return (
    <div className="p-4">
      ...
      <div className="grid gap-3 max-w-md mx-auto">
        {PACKS.map(pack => (
          <div key={pack.id} className="bg-card border-2 border-border rounded p-4">
            <div className="flex justify-between items-start mb-2">
              <div>
                <h3 className="font-heading text-[10px] text-card-foreground">{pack.name}</h3>
                <p className="text-sm text-muted-foreground">{pack.seedCount} seeds</p>
              </div>
              <button
                onClick={() => handleBuyPack(pack.id)}
                disabled={state.money < pack.cost || revealing !== null}
          ...
          <div className="max-w-sm w-full">
            ...
            <div className="grid grid-cols-3 sm:grid-cols-5 gap-3">
              {revealing.map((seed, i) => (
                <div
                  key={seed.id}
                  className={`aspect-square bg-card border-2 border-border rounded flex flex-col items-center justify-center gap-1
